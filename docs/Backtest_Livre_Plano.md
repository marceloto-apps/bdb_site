...
| **5. Validação avançada** — **feita em 26/09/2026** (`docs/Backtest_Livre_Fase5.md`) | `engine/validacao.ts`: holdout selado (última temporada por competição, via manifesto), folds, walk-forward por quantis de datas (com escolha de `$p` no treino), varredura ≤ 200 combos + PBO-lite + heatmap, deflação por tentativas (Šidák + E[max]), Monte Carlo com o staking escolhido, seleção aleatória (Kaunitz), calibração (Brier/log-loss/ECE vs referência); passo 6 na UI + 4 abas no tearsheet; `--validacao` na CLI | Testes com casos sintéticos de resposta conhecida (ex.: estratégia sem edge deve dar p ≈ uniforme) |
